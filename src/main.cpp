...
#if defined(NDEBUG)
# error "Amber cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x329fb758059e99fa4f6e97beffb724bc97ac11e68528f4846685ec82db61747a");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(520503525);
...
const string strMessageMagic = "Amber Signed Message:\n";
...
{
    // Amber: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 560000); // Amber: 560k blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Amber: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Amber: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x80c94923600595277af5c735c2a4b533ac5ab908a6c1017e7a365a243cbabb77");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "if Amber is a color, XAMR is a cryptocurrency";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 70 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046a324625f4710caf69149861680f19ad0b9064963dc60b479239b7aa9612cbad1bfdaf7959d7c84e21a5945bd278d9d6e8daa62b06e62206efe24053dcce4136") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521966827;
        block.nBits    = 520503525;
        block.nNonce   = 2084526218;

        if (fTestNet)
        {
            block.nTime    = 1521966827;
            block.nNonce   = 385273295;
        }
        //// debug print
        ...
        assert(block.hashMerkleRoot == uint256("0x7a8e1b47611a477cc0b47e048d969607c17300a386c386826514555a97e96dc2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa9, 0xc8, 0x6b, 0xcf };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AmberMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AmberMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AmberMiner(CWallet *pwallet)
{
    printf("AmberMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("amber-miner");
    ...
    try { loop {
        ...
        printf("Running AmberMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AmberMiner, pwallet));
}
